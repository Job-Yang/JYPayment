...
@interface ZBApplePayManager()<PKPaymentAuthorizationViewControllerDelegate>
...
@implementation ZBApplePayManager
...
- (void)pay {
    ...
    if(![PKPaymentAuthorizationViewController canMakePayments]) {
        [[ZBRoutable currentVC] showHint:ZBLocalizedString(@"该设备暂不支持Apple Pay", nil)];
        return;
    }
    
    NSDecimalNumber *subtotalAmount = [NSDecimalNumber decimalNumberWithString:[NSString stringWithFormat:@"%f",self.rechargeAmount]];
    PKPaymentSummaryItem *total = [PKPaymentSummaryItem summaryItemWithLabel:ZBLocalizedString(@"ZEB充值", nil) amount:subtotalAmount];
    ...
    [[ZBRoutable currentVC] presentViewController:vc animated:YES completion:nil];
...
- (void *)rechargeSuccess:(void (^)(ZBResponseModel *responseModel))success
...
//    [self rechargeSuccess:^(ZBResponseModel *responseModel) {
